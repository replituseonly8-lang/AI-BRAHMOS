...
Powered by the latest in AI innovation."""
    ...
    success = safe_send_photo_with_caption(
        bot, message.chat.id, "Brahmos.png", welcome_text, keyboard, parse_mode="Markdown"
    )
    if not success:
...
def message_handler(message):
    ...
    try:
        ...
        if user_id in user_waiting_for_edit and message.photo:
            handle_edit_photo(bot, message, user_waiting_for_edit, usage_tracker)
            return
        ...
        if chat_type == "private":
            ...
            if user_id in chat_mode or user_id in user_waiting_for_chat:
                ...
                bot.reply_to(message, """👋 **Hi there!** I'm BrahMos AI.
...
**What would you like to do?**""", parse_mode="Markdown")
        ...
        # Group chat handling
            ...
            if should_respond:
                ...
                ai_response = get_ai_response(text, message.from_user.first_name, message.chat.id, "Group mention/reply")
        ...
        bot.reply_to(message, "❌ **Error:** Something went wrong processing your message.")
